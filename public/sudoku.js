...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function convertTo2DArray(flatString) {
        const size = 9;
        const array = [];
        for (let row = 0; row < size; row++) {
            ...
            for (let col = 0; col < size; col++) {
                ...
                rowArray.push(parseInt(flatString[index], 10));
            }
            array.push(rowArray);
        }
        return array;
    ...
    async function generateBoard() {
        try {
            ...
            // Convert the raw string data into 2D arrays
            board = convertTo2DArray(data.puzzle); // Use 'puzzle' instead of 'board'
            console.log(board)
            solution = convertTo2DArray(data.solution);
            original = board;
            createBoard(board);
        } catch (err) {
